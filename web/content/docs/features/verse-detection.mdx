...
and a **cosine-similarity scan** over 155k+ vectors at
384 dims returns nearest neighbors in a few milliseconds on a
modern CPU.
...
<Callout title="Optional AI Candidate Ranking" type="info">
SabbathCue supports optional candidate re-ranking powered by **DeepSeek** and
**Cerebras (GPT-OSS-120B)** models. When an indirect reference or complex sermon
analogy leaves multiple plausible verse candidates, the model evaluates the context
and adds a recommendation badge to the operator view. It requires your own API key
