...
/// - `/sabbathcue/on_air` (bool argument)
pub fn parse_osc(address: &str, args: &[OscType]) -> Result<RemoteCommand, CommandError> {
    match address {
        "/sabbathcue/next" => Ok(RemoteCommand::Next),
        "/sabbathcue/prev" => Ok(RemoteCommand::Prev),
        "/sabbathcue/show" => Ok(RemoteCommand::Show),
        "/sabbathcue/hide" => Ok(RemoteCommand::Hide),
        "/sabbathcue/theme" => {
...
mod tests {
    ...
    #[test]
    fn parse_osc_next() {
        assert_eq!(parse_osc("/sabbathcue/next", &[]).unwrap(), RemoteCommand::Next);
    }

    #[test]
    fn parse_osc_prev() {
        assert_eq!(parse_osc("/sabbathcue/prev", &[]).unwrap(), RemoteCommand::Prev);
    }

    #[test]
    fn parse_osc_show() {
        assert_eq!(parse_osc("/sabbathcue/show", &[]).unwrap(), RemoteCommand::Show);
    }

    #[test]
    fn parse_osc_hide() {
        assert_eq!(parse_osc("/sabbathcue/hide", &[]).unwrap(), RemoteCommand::Hide);
    }

    #[test]
    fn parse_osc_theme() {
        assert_eq!(
            parse_osc("/sabbathcue/theme", &[OscType::String("Minimal".into())]).unwrap(),
    ...
    fn parse_osc_opacity() {
        assert_eq!(
            parse_osc("/sabbathcue/opacity", &[OscType::Float(0.5)]).unwrap(),
    ...
    fn parse_osc_confidence_from_int_percent() {
        let result = parse_osc("/sabbathcue/confidence", &[OscType::Int(80)]).unwrap();
    ...
    fn parse_osc_on_air_from_int() {
        assert_eq!(
            parse_osc("/sabbathcue/on_air", &[OscType::Int(1)]).unwrap(),
    ...
    fn parse_osc_unknown_address_errors() {
        let result = parse_osc("/sabbathcue/unknown", &[]);
    ...
    fn parse_osc_opacity_missing_arg_errors() {
        let result = parse_osc("/sabbathcue/opacity", &[]);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    fn parse_osc_theme_missing_arg_errors() {
        let result = parse_osc("/sabbathcue/theme", &[]);
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    fn parse_osc_on_air_missing_arg_errors() {
        let result = parse_osc("/sabbathcue/on_air", &[]);
        assert!(result.is_err());
    }

    #[test]
    fn parse_osc_confidence_missing_arg_errors() {
        let result = parse_osc("/sabbathcue/confidence", &[]);
        assert!(result.is_err());
    }
    ...
    fn parse_osc_on_air_from_float() {
        ...
        assert_eq!(
            parse_osc("/sabbathcue/on_air", &[OscType::Float(1.0)]).unwrap(),
            RemoteCommand::OnAir(true)
        );
        assert_eq!(
            parse_osc("/sabbathcue/on_air", &[OscType::Float(0.0)]).unwrap(),
    ...
    fn parse_osc_opacity_from_int_percent() {
        ...
        assert_eq!(
            parse_osc("/sabbathcue/opacity", &[OscType::Int(50)]).unwrap(),
            RemoteCommand::Opacity(0.5)
        );
    }
    ...
    fn parse_osc_on_air_from_string() {
        ...
        assert_eq!(
            parse_osc("/sabbathcue/on_air", &[OscType::String("on".into())]).unwrap(),
            RemoteCommand::OnAir(true)
        );
    }
}
